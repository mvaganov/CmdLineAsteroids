...
namespace asteroids {
	public class SpacePartition<T> {
		...
		public List<CollisionLogic.ToResolve> DoCollisionLogic(Dictionary<CollisionPair, List<CollisionLogic.Function>> rules) {
			List<CollisionLogic.ToResolve> result = null;
			if (list != null) {
				result = CollisionLogic.DoCollisionLogic(list as IList<ICollidable>, rules);
			}
			if (subpartition != null) {
				...
				for (int r = 0; r < rows; ++r) {
					for (int c = 0; c < cols; ++c) {
						List<CollisionLogic.ToResolve> hits = subpartition[r,c].DoCollisionLogic(rules);
						if (hits == null) {
							continue;
						}
						if (result == null) {
		...
		public void DoCollisionLogicAndResolve(Dictionary<CollisionPair, List<CollisionLogic.Function>> rules) {
			...
			//while (FindDuplicateIndex(collisionsToResolve, out int a, out int b)) {
			//	if (collisionsToResolve[a].collision.self.TypeId == 3) {
			//		Log.i("projectile dup, prepare for crash?");
			//	}
			//	collisionsToResolve.RemoveAt(b);
			//}
			collisionsToResolve.ForEach(c => c.resolution.Invoke());
		}
